...
test("GUI exposes structured real-time APK build status", () => {
  ...
  assert.match(main, /bundleRelease/);
  assert.match(main, /app-release\.aab/);
  ...
  assert.match(page, /id="keystore-alias-suggestions"/);
  assert.match(page, /id="includeAppBundle"/);
  assert.match(page, /id="download-aab-btn"/);
